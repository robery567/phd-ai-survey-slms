...
nb12_cells = [
    md(
        ...
        "\n",
        "**Runtime:** ~3-4 GPU-hours on an 80 GB A100 (50 behaviors × 3 models × ~2-3 min/run at 250 steps × 1024 search-width × n_replace=2 with prefix cache and early-stop, using SDPA attention). The candidate batch is processed in 2 chunks of 512 (`batch_size=512`), pre-sized to stay below the auto-finder's OOM cliff so we don't pay a per-step recover cost. If you're on a 40 GB A100, drop `gcg_batch_size` to 256.\n",
