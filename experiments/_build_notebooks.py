...
nb12_cells = [
    md(
        ...
        "\n",
        "**Runtime:** ~6-8 GPU-hours on an 80 GB A100 (50 behaviors × 3 models × ~3-4 min/run at 250 steps × 1024 search-width with prefix cache and early-stop, using SDPA attention). The default 80 GB Colab Pro A100 has ample headroom for the larger search width; if you're on a 40 GB A100, reduce `gcg_search_width` to 512 in the REPRO block.\n",
    ...
    code(
        ...
        "import nanogcg as _ngcg\n",
        "_ngcg_ver = getattr(_ngcg, '__version__', 'unknown')\n",
