...
nb12_cells = [
    md(
        ...
        "\n",
        "**Runtime:** ~3-5 GPU-hours on an 80 GB A100 (50 behaviors × 3 models × ~2-3 min/run at 250 steps × 2048 search-width × n_replace=2 with prefix cache and early-stop, using SDPA attention). The full `search_width` is processed in a single forward pass per step (`batch_size=None`), which uses the available 56 GB of VRAM the previous batched config was leaving idle. If you're on a 40 GB A100, drop `gcg_search_width` to 1024 in the REPRO block.\n",
    ...
    code(
        ...
        "    'gcg_search_width': 2048,     # 4x default. With ~56 GB free VRAM after model load, the candidate batch easily fits in one forward pass.\n",
        "    'gcg_batch_size': None,       # None = process all `search_width` candidates in a single forward pass (find_executable_batch_size auto-halves on OOM). Sub-batching at 256 was leaving 75% of the A100 idle.\n",
        "    'gcg_topk': 256,              # default\n",
        "    'gcg_n_replace': 2,           # 2x default. Each candidate replaces 2 tokens per step instead of 1 -> ~2x faster convergence on most behaviors.\n",
