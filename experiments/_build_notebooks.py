...
nb12_cells = [
    md(
        ...
        "\n",
        "**Runtime:** ~6-8 GPU-hours on an 80 GB A100 (50 behaviors × 3 models × ~3-4 min/run at 250 steps × 1024 search-width with the prefix-cache monkey-patch and early-stop, using SDPA attention). The default 80 GB Colab Pro A100 has ample headroom for the larger search width; if you're on a 40 GB A100, reduce `gcg_search_width` to 512 in the REPRO block.\n",
    ...
    code(
        ...
        "# sampling, slightly different config surface) which our patch can't repair.\n",
        "# Pinning transformers to a compatible version lets us install nanoGCG 0.3.0\n",
        "# AND skip the DynamicCache compatibility patch entirely - the legacy tuple\n",
        "# cache format still works on transformers <=4.47, which is what nanoGCG\n",
        "# expects. Net: simpler, faster, no monkey-patching.\n",
    ...
    code(
        ...
        "    'gcg_search_width': 1024,     # bumped from 512 (default); 80 GB A100 has plenty of headroom\n",
        "    'gcg_batch_size': 256,        # process candidates in chunks of 256 to keep peak VRAM bounded\n",
        "    'gcg_topk': 256,              # default\n",
        "    'gcg_early_stop': True,       # stop once the model emits the target sequence verbatim\n",
        "    'gcg_use_prefix_cache': True, # re-enabled via the DynamicCache monkey-patch below; ~2x faster\n",
    ...
    code(
        "# --- Speedups -----------------------------------------------------\n",
        "# (1) Patch nanoGCG to handle the new transformers DynamicCache.\n",
        "# nanoGCG v0.3.0 indexes prefix_cache[i] which assumes a tuple-of-tuples;\n",
        "# transformers >=4.43 returns a DynamicCache. We replace nanoGCG.GCG.run\n",
        "# with a copy that calls to_legacy_cache() right after the prefix_cache\n",
        "# is populated. Result: use_prefix_cache=True works again, ~2x speedup\n",
        "# vs running with the cache disabled.\n",
        "import copy\n",
        "from tqdm import tqdm\n",
        "import nanogcg.gcg as _ng\n",
        "\n",
        "def _patched_run(self, messages, target):\n",
        "    \"\"\"Copy of nanogcg.gcg.GCG.run with one fix: convert DynamicCache to a\n",
        "    legacy tuple-of-tuples right after model() returns it, so the rest of\n",
        "    nanoGCG (which indexes prefix_cache[i]) works unchanged.\n",
        "    \"\"\"\n",
        "    import torch\n",
        ...
        "            # *** THE FIX *** convert DynamicCache -> tuple-of-tuples\n",
        "            if hasattr(pkv, 'to_legacy_cache'):\n",
        "                pkv = pkv.to_legacy_cache()\n",
        "            self.prefix_cache = pkv\n",
        "    self.target_ids = target_ids\n",
        "    self.before_embeds = before_embeds\n",
        "    self.after_embeds = after_embeds\n",
        "    self.target_embeds = target_embeds\n",
        "    if getattr(config, 'probe_sampling_config', None):\n",
        "        # We don't use probe sampling, but keep the path intact for completeness.\n",
        "        raise RuntimeError('Probe sampling not supported by this patched run().')\n",
        ...
        "        with torch.no_grad():\n",
        "            sampled_ids = _ng.sample_ids_from_grad(\n",
        "                optim_ids.squeeze(0),\n",
        ...
        "                sampled_ids = _ng.filter_ids(sampled_ids, tokenizer)\n",
        ...
        "        if self.stop_flag:\n",
        "            _ng.logger.info('Early stopping due to finding a perfect match.')\n",
        "            break\n",
        "    min_loss_index = losses.index(min(losses))\n",
        "    return _ng.GCGResult(\n",
        ...
        "    )\n",
        "\n",
        "_ng.GCG.run = _patched_run\n",
        "print('Patched nanogcg.gcg.GCG.run for DynamicCache compatibility.')\n",
        "\n",
        "# (2) Use SDPA for the target model. We considered FlashAttention 2 but the\n",
