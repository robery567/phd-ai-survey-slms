...
nb12_cells = [
    md(
        ...
        "\n",
        "**Runtime:** ~5-7 GPU-hours on an 80 GB A100 (50 behaviors × 3 models × ~2-3 min/run at 250 steps × 1024 search-width with FlashAttention 2, prefix cache, and early-stop). The default 80 GB Colab Pro A100 has ample headroom for the larger search width; if you're on a 40 GB A100, reduce `gcg_search_width` to 512 in the REPRO block.\n",
    ...
    code(
        ...
        "# FlashAttention 2 for ~2-3x faster forward passes on A100. Install is a no-op\n",
        "# if a compatible wheel is already cached in the runtime image.\n",
        "!pip install flash-attn --no-build-isolation -q 2>/dev/null || echo 'flash-attn build skipped (will fall back to SDPA)'\n",
    ...
    code(
        "# --- Speedups -----------------------------------------------------\n",
        "# (1) Monkey-patch nanoGCG to handle the new transformers DynamicCache.\n",
        "# nanoGCG v0.3.0 indexes prefix_cache[i] which assumes a tuple-of-tuples;\n",
        "# transformers >=4.43 returns a DynamicCache. We rewrite the prefix-cache\n",
        "# population step to convert to legacy tuples up front, then everything\n",
        "# downstream works unchanged. Result: use_prefix_cache=True works again,\n",
        "# which is roughly a 2x speedup vs running with the cache disabled.\n",
        "import nanogcg.gcg as _ng\n",
        "_orig_init_buffer = _ng.GCG.init_buffer\n",
        "def _patched_init_buffer(self, *args, **kwargs):\n",
        "    out = _orig_init_buffer(self, *args, **kwargs)\n",
        "    pc = getattr(self, 'prefix_cache', None)\n",
        "    if pc is not None and hasattr(pc, 'to_legacy_cache'):\n",
        "        # Convert DynamicCache -> tuple-of-(key, value) tuples.\n",
        "        self.prefix_cache = pc.to_legacy_cache()\n",
        "    return out\n",
        "_ng.GCG.init_buffer = _patched_init_buffer\n",
        "print('Patched nanoGCG prefix_cache for DynamicCache compatibility.')\n",
        "\n",
        "# (2) Try to use FlashAttention 2 for the target model. SDPA is the safe fallback.\n",
        "try:\n",
        ...
        "except ImportError:\n",
        "    _ATTN_IMPL = 'sdpa'\n",
        "    print(f'flash_attn not installed; using attn_implementation={_ATTN_IMPL!r} (still fast on A100).')\n",
        "\n",
        ...
        "        # Some models reject flash_attention_2; fall back to SDPA.\n",
        "        model = AutoModelForCausalLM.from_pretrained(\n",
